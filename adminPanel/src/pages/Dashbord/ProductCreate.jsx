import React, { useState } from "react";
import { useSelector } from "react-redux";
import { Link } from "react-router-dom";
import Select from "react-select";
import userFromField from "../../hooks/userFromField";
import { MdClose } from "react-icons/md";

const ProductCreate = () => {
  const [productType, setProductType] = useState("simple");
  const [tags, setTags] = useState(null);
  const [cats, setCats] = useState([]);
  const [productPhoto, setProductPhoto] = useState([]);
  const { catagory, brand, tag } = useSelector((item) => item.product);

  const { input, handleInputChange, resetForm, setinput } = userFromField({
    name: "",
    brand: "",
    shortDesc: "",
    longDesc: "",
    stock: 0,
    salePrice: "",
    regularPrice: "",
    link: null,
  });

  let tagOption = [];
  tag?.forEach((element) => {
    tagOption.push({ value: element._id, label: element.name });
  });

  // handle photo manage
  const handleProductPhoto = (e) => {
    const uploadFiles = Array.from(e.target.files);
    setProductPhoto((prev) => [...prev, ...uploadFiles]);
  };

  // handle category
  const handleCatChange = (e) => {
    const seletedCatList = [...cats];

    if (cats?.includes(e.target.value)) {
      seletedCatList.splice(seletedCatList.indexOf(e.target.value), 1);
    } else {
      seletedCatList.push(e.target.value);
    }

    setCats(seletedCatList);
  };
  console.log(productPhoto);
  return (
    <div>
      <Link
        className="hover:no-underline   font-semibold text-lg bg-blue-400 w-fit px-4 py-1 text-white rounded-md mb-3 cursor-pointer   "
        to={"/product"}
      >
        Back
      </Link>

      <div className="w-full sm:flex gap-4 mt-3">
        <div className="sm:w-8/12 mb-3 sm:mb-0 p-3 rounded-md bg-slate-100 space-y-2">
          <p className="font-semibold ">Add new product</p>

          <p>Name :</p>
          <input
            type="text"
            className="w-full p-1 border rounded-md"
            name="name"
            onChange={handleInputChange}
            value={input.value}
          />

          <p>Product Type :</p>
          <select
            name=""
            id=""
            onChange={(e) => setProductType(e.target.value)}
            className="w-full p-2 bg-blue-400 text-white rounded-md font-semibold  "
          >
            <option value="simple">simple</option>
            <option value="variable">variable</option>
            <option value="group">group</option>
            <option value="external">external</option>
          </select>

          {productType == "simple" && (
            <div className="p-4 bg-blue-100 space-y-2 font-semibold rounded-md">
              <div className="flex justify-between ">
                <label htmlFor="">Regular price</label>
                <input
                  className="w-8/12 rounded-md p-1"
                  type="text"
                  name="regularPrice"
                  onChange={handleInputChange}
                  value={input.regularPrice}
                  id=""
                />
              </div>
              <div className="flex justify-between">
                <label htmlFor="">Sale price</label>
                <input
                  type="text"
                  className="w-8/12 rounded-md p-1"
                  name="salePrice"
                  onChange={handleInputChange}
                  value={input.salePrice}
                  id=""
                />
              </div>
              <div className="flex justify-between">
                <label htmlFor="">Stock</label>
                <input
                  type="text"
                  className="w-8/12 rounded-md p-1"
                  name="stock"
                  onChange={handleInputChange}
                  value={input.stock}
                  id=""
                />
              </div>
            </div>
          )}
          {productType == "external" && (
            <div className="p-4 bg-red-100 space-y-2 font-semibold rounded-md">
              <div className="flex justify-between ">
                <label htmlFor="">Regular price</label>
                <input
                  className="w-8/12 rounded-md p-1"
                  type="text"
                  name="regularPrice"
                  onChange={handleInputChange}
                  value={input.regularPrice}
                  id=""
                />
              </div>
              <div className="flex justify-between">
                <label htmlFor="">Sale price</label>
                <input
                  type="text"
                  className="w-8/12 rounded-md p-1"
                  name="salePrice"
                  onChange={handleInputChange}
                  value={input.salePrice}
                  id=""
                />
              </div>
              <div className="flex justify-between">
                <label htmlFor="">Link</label>
                <input
                  type="text"
                  className="w-8/12 rounded-md p-1"
                  name="link"
                  onChange={handleInputChange}
                  value={input.link}
                  id=""
                />
              </div>
            </div>
          )}

          <p>shortDesc :</p>
          <textarea
            type="text"
            className="w-full p-1 border rounded-md"
            name="shortDesc"
            value={input.shortDesc}
            onChange={handleInputChange}
          />
          <p>longDesc :</p>
          <textarea
            type="text"
            className="w-full p-1 border rounded-md"
            name="longDesc"
            value={input.longDesc}
            onChange={handleInputChange}
          />
        </div>

        {productType == "simple" && (
          <div className="sm:w-4/12 rounded-md p-2 bg-slate-100   ">
            <div className="">
              <label htmlFor="">Photo</label>
              <input
                type="file"
                name=""
                multiple
                id=""
                onChange={handleProductPhoto}
              />
              <div className="grid grid-cols-3 gap-3  ">
                <div className="   relative  p-1 rounded-md border bg-white   duration-300 ">
                  <MdClose className="absolute top-2 right-2 bg-gray-100 hover:text-white text-black rounded-md hover:cursor-pointer text-xl duration-300 hover:text-2xl hover:bg-red-600" />
                  <img
                    className="rounded-md"
                    src="data:image/jpeg;base64,"
                    alt=""
                  />
                </div>
                <div className="   relative  p-1 rounded-md border bg-white   duration-300 ">
                  <MdClose className="absolute top-2 right-2 bg-gray-100 hover:text-white text-black rounded-md hover:cursor-pointer text-xl duration-300 hover:text-2xl hover:bg-red-600" />
                  <img
                    className="rounded-md"
                    src="data:image/jpeg;base64,"
                    alt=""
                  />
                </div>
                <div className="  relative  p-1 rounded-md border bg-white   duration-300 ">
                  <MdClose className="absolute top-2 right-2 bg-gray-100 hover:text-white text-black rounded-md hover:cursor-pointer text-xl duration-300 hover:text-2xl hover:bg-red-600" />
                  <img
                    className="rounded-md"
                    src="data:image/jpeg;base64,"
                    alt=""
                  />
                </div>
              </div>
            </div>

            <p className="mt-3 ">Catagory :</p>
            <div className="flex flex-wrap gap-2  my-2">
              {catagory?.map((item) => {
                return (
                  <label key={item._id} htmlFor={item._id}>
                    <p className="cursor-pointer bg-slate-300 px-2 rounded-md">
                      <input
                        onChange={handleCatChange}
                        checked={cats?.includes(item._id) ? true : false}
                        type="checkbox"
                        id={item._id}
                        value={item._id}
                      />{" "}
                      <span>{item.name}</span>
                    </p>
                  </label>
                );
              })}
            </div>
            <p className="mt-3 mb-2">Brand :</p>
            <select
              name="brand"
              value={input.brand}
              onChange={handleInputChange}
              id=""
              className="w-full p-2 bg-blue-400 text-white rounded-md font-semibold  "
            >
              <option value="">--Select--</option>
              {brand?.map((item) => {
                return (
                  <option key={item._id} value={item._id}>
                    {item.name}
                  </option>
                );
              })}
            </select>
            <p className="mt-3 mb-2">Tag :</p>
            <Select
              name="tags"
              value={tags}
              onChange={(tags) => setTags(tags)}
              options={tagOption}
              isMulti
            />
          </div>
        )}
        {productType == "external" && (
          <div className="sm:w-4/12 rounded-md p-2 bg-sky-400"></div>
        )}
      </div>
    </div>
  );
};

export default ProductCreate;
